  ...
  for (const r of atRules) {
    if (r.nodes !== void 0) {
      const families = (
        ...
        r.nodes.filter((node) => node.type === "decl" && node.prop === "font-family")
      );
          ...
          css.walk((node) => {
            ...
            if (type === rule && namespace && node.selector.includes("|")) {
              if (node.selector.includes("[")) {
                postcss_selector_parser_default((ast) => {
                  ast.walkAttributes(({ namespace: ns }) => {
                    namespaceCache.rules = namespaceCache.rules.concat(ns);
                  });
                }).process(node.selector);
              } else {
                namespaceCache.rules = namespaceCache.rules.concat(node.selector.split("|")[0]);
              }
              return;
            }
            if (type === decl) {
              ...
              if (counterStyle && /list-style|system/.test(prop)) {
                counterStyleCache.values = counterStyleCache.values.concat(splitValues(node, comma2, space2));
              }
              if (fontFace && node.parent !== void 0 && node.parent.type === rule && /font(|-family)/.test(prop)) {
                fontCache.values = fontCache.values.concat(comma2(node.value.toLowerCase()));
              }
              if (keyframes && /animation/.test(prop)) {
                keyframesCache.values = keyframesCache.values.concat(splitValues(node, comma2, space2));
              }
              return;
            }
            if (type === atrule) {
...
var postcss_discard_unused_default = pluginCreator;
export {
  postcss_discard_unused_default as default
};
