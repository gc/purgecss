...
var Processor = class {
  ...
  _root(rule, options = {}) {
    let parser2 = new Parser(rule, this._parseOptions(options));
    return parser2.root;
  }
...
var comment2 = /* @__PURE__ */ __name((opts) => new Comment(opts), "comment");
...
var parser = /* @__PURE__ */ __name((processor) => new Processor(processor), "parser");
Object.assign(parser, selectors_exports);
var postcss_selector_parser_default = parser;

// src/postcss-discard-comments/index.js
function CommentRemover(options) {
  this.options = options;
}
__name(CommentRemover, "CommentRemover");
CommentRemover.prototype.canRemove = function(comment3) {
  const remove = this.options.remove;
  if (remove) {
    return remove(comment3);
  } else {
    const isImportant = comment3.indexOf("!") === 0;
    if (!isImportant) {
      return true;
    }
...
__name(commentParser, "commentParser");
function pluginCreator(opts = {}) {
  ...
  __name(matchesComments, "matchesComments");
  function replaceComments(source, space2, separator = " ") {
    ...
    const result = space2(parsed).join(" ");
    replacerCache.set(key, result);
    return result;
  }
  __name(replaceComments, "replaceComments");
  function replaceCommentsInSelector(source, space2) {
    const key = source + "@|@";
    if (replacerCache.has(key)) {
      return replacerCache.get(key);
    }
    const processed = postcss_selector_parser_default((ast) => {
      ast.walk((node) => {
        ...
        const rawSpaceAfter = replaceComments(node.rawSpaceAfter, space2, "");
        const rawSpaceBefore = replaceComments(node.rawSpaceBefore, space2, "");
    ...
    const result = space2(processed).join(" ");
    replacerCache.set(key, result);
    return result;
  }
  __name(replaceCommentsInSelector, "replaceCommentsInSelector");
  return {
      ...
      css.walk((node) => {
        ...
        if (node.type === "rule") {
          if (node.raws.selector && node.raws.selector.raw) {
            node.raws.selector.raw = replaceCommentsInSelector(node.raws.selector.raw, list.space);
          } else if (node.selector && node.selector.includes("/*")) {
            node.selector = replaceCommentsInSelector(node.selector, list.space);
          }
          return;
        }
        if (node.type === "atrule") {
...
var postcss_discard_comments_default = pluginCreator;
export {
  postcss_discard_comments_default as default
};
