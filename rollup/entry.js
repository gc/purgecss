...
var Root2 = class extends Container {
  ...
  static registerLazyResult =  __name((dependant) => {
    LazyResult = dependant;
  }, "registerLazyResult");
  static registerProcessor =  __name((dependant) => {
    Processor = dependant;
  }, "registerProcessor");
};
...
var LazyResult3 = class _LazyResult {
  ...
  prepareVisitors() {
    this.listeners = {};
    const add =  __name((plugin2, type, cb) => {
...
postcss2.plugin =  __name(function plugin(name, initializer) {
...
var cloneNode2 =  __name(function(obj, parent) {
...
var merge =  __name(function merge2(options, defaults) {
  if (!options) {
    return defaults;
  }
  var result = {};
...
var cssesc =  __name(function cssesc2(string2, options) {
    ...
    while (pos < len) {
      ...
      switch (token[FIELDS.TYPE]) {
        ...
        case combinator:
          ...
          break;
        case word:
          if (next && this.content(next) === "|" && (attr[pos + 2] && attr[pos + 2][FIELDS.TYPE] !== equals) &&
...
var Processor4 = class {
  ...
  constructor(func, options) {
    this.func = func ||  __name(function noop2() {
    }, "noop");
    this.funcRes = null;
...
var attribute =  __name((opts) => new Attribute(opts), "attribute");
var className =  __name((opts) => new ClassName(opts), "className");
var combinator2 =  __name((opts) => new Combinator(opts), "combinator");
var comment2 =  __name((opts) => new Comment2(opts), "comment");
var id =  __name((opts) => new ID2(opts), "id");
var nesting =  __name((opts) => new Nesting(opts), "nesting");
var pseudo =  __name((opts) => new Pseudo(opts), "pseudo");
var root =  __name((opts) => new Root3(opts), "root");
var selector =  __name((opts) => new Selector(opts), "selector");
var string =  __name((opts) => new String2(opts), "string");
var tag =  __name((opts) => new Tag(opts), "tag");
var universal =  __name((opts) => new Universal(opts), "universal");
var IS_TYPE = {
  [ATTRIBUTE]: true,
  [CLASS]: true,
  [COMBINATOR]: true,
...
var parser =  __name((processor) => new Processor4(processor), "parser");
...
var defaultOptions = {
  css: [],
  content: [],
  defaultExtractor:  __name((content) => content.match(/[A-Za-z0-9_-]+/g) || [], "defaultExtractor"),
  extractors: [],
  fontFace: false,
  keyframes: false,
  rejected: false,
...
var PurgeCSS = class {
  ...
  shouldKeepSelector(selector2, selectorsFromExtractor) {
    if ( isInPseudoClass(selector2) && ! isInPseudoClassWhereOrIs(selector2)) {
      return true;
    }
    if ( isPseudoClassAtRootLevel(selector2)) {
      return true;
    }
...
var purgeCSSPlugin =  __name((opts) => {
...
function consumeStringToken(ctx, reader) {
  ...
  while (true) {
    ...
    if (typeof next === "undefined") {
      const token = ["string-token" , reader.source.slice(reader.representationStart, reader.representationEnd + 1), reader.representationStart, reader.representationEnd, { value: result }];
      ctx.onParseError(new ParseErrorWithToken(
    ...
    if (isNewLine(next)) {
      ...
      const token = ["bad-string-token" , reader.source.slice(reader.representationStart, reader.representationEnd + 1), reader.representationStart, reader.representationEnd, void 0];
      ctx.onParseError(new ParseErrorWithToken(
    ...
    if (next === first) {
      return ["string-token" , reader.source.slice(reader.representationStart, reader.representationEnd + 1), reader.representationStart, reader.representationEnd, { value: result }];
    }
...
function checkIfThreeCodePointsWouldStartAUnicodeRange(reader) {
  if (
    (reader.source.codePointAt(reader.cursor) === LATIN_SMALL_LETTER_U || reader.source.codePointAt(reader.cursor) === LATIN_CAPITAL_LETTER_U) &&
...
function tokenizer2(input, options) {
  ...
  function nextToken() {
    ...
    switch (peeked) {
      case COMMA:
        reader.advanceCodePoint();
        return ["comma-token" , ",", reader.representationStart, reader.representationEnd, void 0];
      case COLON2:
        reader.advanceCodePoint();
        return ["colon-token" , ":", reader.representationStart, reader.representationEnd, void 0];
      case SEMICOLON2:
        reader.advanceCodePoint();
        return ["semicolon-token" , ";", reader.representationStart, reader.representationEnd, void 0];
      case LEFT_PARENTHESIS:
        reader.advanceCodePoint();
        return ["(-token" , "(", reader.representationStart, reader.representationEnd, void 0];
      ...
      case FULL_STOP:
        ...
        reader.advanceCodePoint();
        return ["delim-token" , reader.source[reader.representationStart], reader.representationStart, reader.representationEnd, {
      ...
      case HYPHEN_MINUS:
        ...
        if (checkIfThreeCodePointsWouldStartCDC(reader)) {
          reader.advanceCodePoint(3);
          return ["CDC-token" , "-->", reader.representationStart, reader.representationEnd, void 0];
        }
        if (checkIfThreeCodePointsWouldStartAnIdentSequence(ctx, reader)) {
          return consumeIdentLikeToken(ctx, reader);
        }
        reader.advanceCodePoint();
        return ["delim-token" , "-", reader.representationStart, reader.representationEnd, {
          value: "-"
        }];
      case LESS_THAN_SIGN:
        if (checkIfFourCodePointsWouldStartCDO(reader)) {
          reader.advanceCodePoint(4);
          return ["CDO-token" , "<!--", reader.representationStart, reader.representationEnd, void 0];
        }
        reader.advanceCodePoint();
        return ["delim-token" , "<", reader.representationStart, reader.representationEnd, {
          value: "<"
      ...
      case COMMERCIAL_AT:
        ...
        if (checkIfThreeCodePointsWouldStartAnIdentSequence(ctx, reader)) {
          const identSequence = consumeIdentSequence(ctx, reader);
          return ["at-keyword-token" , reader.source.slice(reader.representationStart, reader.representationEnd + 1), reader.representationStart, reader.representationEnd, {
            value: String.fromCodePoint(...identSequence)
          }];
        }
        return ["delim-token" , "@", reader.representationStart, reader.representationEnd, {
      ...
      case REVERSE_SOLIDUS: {
        ...
        reader.advanceCodePoint();
        const token = ["delim-token" , "\\", reader.representationStart, reader.representationEnd, {
          value: "\\"
        }];
        ctx.onParseError(new ParseErrorWithToken(
...
export { postcss_minify_default as postCSSMinifyPlugin, postcss_default as postcss, purgeCSSPlugin };
