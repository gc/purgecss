import selectorParser from "../postcss-selector-parser/index.js";
const atrule = 'atrule';
const decl = 'decl';
const rule = 'rule';
/**
...
function filterFont({ atRules, values }, comma) {
    ...
    for (const r of atRules) {
        if (r.nodes !== undefined) {
            ...
            const families = /** @type {import('postcss').Declaration[]} */ (r.nodes.filter((node) => node.type === 'decl' && node.prop === 'font-family'));
                    ...
                    css.walk((node) => {
                        ...
                        if (type === decl) {
                            ...
                            if (counterStyle && /list-style|system/.test(prop)) {
                                counterStyleCache.values = counterStyleCache.values.concat(splitValues(node, comma, space));
                            }
                            if (fontFace &&
                            ...
                            if (keyframes && /animation/.test(prop)) {
                                keyframesCache.values = keyframesCache.values.concat(splitValues(node, comma, space));
                            }
                            return;
                        }
                        if (type === atrule) {
